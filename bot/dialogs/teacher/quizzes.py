...
class TeacherQuizSG(StatesGroup):
    quiz_list = State()
    quiz_detail = State()
    enter_title = State()
...
async def on_quiz_selected(callback: CallbackQuery, widget: Any, manager: DialogManager, item_id: str) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.quiz_detail)
...
async def on_new_quiz(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.enter_title)
...
async def on_title_entered(message: Message, widget: Any, manager: DialogManager, value: str) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.enter_description)
...
async def on_description_entered(message: Message, widget: Any, manager: DialogManager, value: str) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.enter_time_limit)
...
async def _create_quiz_and_go_to_questions(manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.question_list)
...
async def on_add_question(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.select_qtype)
...
async def on_qtype_single(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.enter_qtext)
...
async def on_qtext_entered(message: Message, widget: Any, manager: DialogManager, value: str) -> None:
    ...
    if manager.dialog_data.get("question_type") == "text":
        ...
        await manager.switch_to(TeacherQuizSG.enter_options)
...
async def on_options_done(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    await manager.switch_to(TeacherQuizSG.mark_correct)
...
async def on_delete_quiz(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.quiz_list)


async def on_assign(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    await manager.switch_to(TeacherQuizSG.assign_target)
...
async def on_assign_to_group(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.assign_group)
...
async def on_assign_to_student(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.assign_student)
...
async def on_group_picked(callback: CallbackQuery, widget: Any, manager: DialogManager, item_id: str) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.assign_deadline)
...
async def on_deadline_entered(message: Message, widget: Any, manager: DialogManager, value: str) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.assign_confirm)
...
async def on_view_results(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    await manager.switch_to(TeacherQuizSG.results_list)
...
async def on_result_selected(callback: CallbackQuery, widget: Any, manager: DialogManager, item_id: str) -> None:
    ...
    if idx < len(results):
        ...
        await manager.switch_to(TeacherQuizSG.result_detail)


async def on_import_excel(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    await manager.switch_to(TeacherQuizSG.import_upload)
...
async def on_xlsx_received(message: Message, widget: Any, manager: DialogManager) -> None:
    ...
    await manager.switch_to(TeacherQuizSG.import_confirm)
...
dialog = Dialog(
    Window(
        ...
        state=TeacherQuizSG.quiz_list,
        getter=get_quiz_list,
    ),
    Window(
        Format("📝 <b>{title}</b>\n📄 {description}\n⏱ {time_limit}\n❓ {q_count} питань"),
        Button(Const("❓ Питання"), id="btn_questions", on_click=_back(TeacherQuizSG.question_list)),
        ...
        Button(Const("← Назад"), id="back_to_list", on_click=_back(TeacherQuizSG.quiz_list)),
        state=TeacherQuizSG.quiz_detail,
        getter=get_quiz_detail,
    ),
    Window(
        Const("📝 Введіть назву тесту:"),
        TextInput(id="title_input", on_success=on_title_entered),
        Button(Const("← Скасувати"), id="cancel_title", on_click=_back(TeacherQuizSG.quiz_list)),
        state=TeacherQuizSG.enter_title,
    ),
    Window(
        ...
        state=TeacherQuizSG.enter_description,
    ),
    Window(
        ...
        state=TeacherQuizSG.enter_time_limit,
    ),
    Window(
        Const("❓ Питання тесту:"),
        ScrollingGroup(
        ...
        Row(
            ...
            Button(Const("← Тест"), id="back_detail", on_click=_back(TeacherQuizSG.quiz_detail)),
        ),
        state=TeacherQuizSG.question_list,
    ...
    Window(
        ...
        Button(Const("← Назад"), id="back_qlist", on_click=_back(TeacherQuizSG.question_list)),
        state=TeacherQuizSG.select_qtype,
    ),
    Window(
        Const("✏️ Введіть текст питання:"),
        TextInput(id="qtext_input", on_success=on_qtext_entered),
        Button(Const("← Назад"), id="back_qtype", on_click=_back(TeacherQuizSG.select_qtype)),
        state=TeacherQuizSG.enter_qtext,
    ),
    Window(
        ...
        Button(Const("← Назад"), id="back_qtext2", on_click=_back(TeacherQuizSG.enter_qtext)),
        state=TeacherQuizSG.enter_options,
    ...
    Window(
        ...
        Button(Const("← Назад"), id="back_opts", on_click=_back(TeacherQuizSG.enter_options)),
        state=TeacherQuizSG.mark_correct,
    ...
    Window(
        ...
        Button(Const("← Назад"), id="back_detail2", on_click=_back(TeacherQuizSG.quiz_detail)),
        state=TeacherQuizSG.assign_target,
    ),
    Window(
        Const("Оберіть групу:"),
        ScrollingGroup(
        ...
        Button(Const("← Назад"), id="back_assign", on_click=_back(TeacherQuizSG.assign_target)),
        state=TeacherQuizSG.assign_group,
    ...
    Window(
        ...
        Button(Const("← Назад"), id="back_assign2", on_click=_back(TeacherQuizSG.assign_target)),
        state=TeacherQuizSG.assign_student,
    ...
    Window(
        ...
        state=TeacherQuizSG.assign_deadline,
    ),
    Window(
        Format("📤 Призначити тест\n\nКому: {assign_target}\nДедлайн: {assign_deadline}\n\nПідтвердити?"),
        Row(
            Button(Const("✅ Так"), id="confirm_assign", on_click=on_confirm_assign),
            Button(Const("← Скасувати"), id="cancel_assign", on_click=_back(TeacherQuizSG.quiz_detail)),
        ),
        state=TeacherQuizSG.assign_confirm,
    ...
    Window(
        ...
        Button(Const("← Назад"), id="back_detail3", on_click=_back(TeacherQuizSG.quiz_detail)),
        state=TeacherQuizSG.results_list,
        getter=get_results_list,
    ),
    Window(
        Format("📋 Відповіді студента:\n\n{detail_text}"),
        Button(Const("← Назад"), id="back_results", on_click=_back(TeacherQuizSG.results_list)),
        state=TeacherQuizSG.result_detail,
    ...
    Window(
        ...
        Button(Const("← Назад"), id="back_list2", on_click=_back(TeacherQuizSG.quiz_list)),
        state=TeacherQuizSG.import_upload,
    ),
    Window(
        ...
        Button(Const("← Скасувати"), id="cancel_import", on_click=_back(TeacherQuizSG.import_upload)),
        state=TeacherQuizSG.import_confirm,
